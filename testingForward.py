...
if __name__ == "__main__":
    ...
    # a1 = psi(n, x)
    a1 = pymiediff.an(x, y, n, m1, m2)
    b1 = pymiediff.bn(x, y, n, m1, m2)

    # a1 = pymiediff.psi(x, n)
    # b1 = pymiediff.chi(x, n)


    x_plot = x.detach().numpy().squeeze()
    a1_plot = a1.detach().numpy().squeeze()
    b1_plot = b1.detach().numpy().squeeze()

    plt.plot(x_plot, a1_plot)
    plt.plot(x_plot, b1_plot)
    plt.show()
