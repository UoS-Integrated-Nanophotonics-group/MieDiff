...
input2 = torch.tensor(0.5, dtype=torch.float64, requires_grad=True)  # m
...
def test_fun(z, n, m):
    '''
    Test composite function with 2 inputs (z and m) that require grad
    '''
    return jn(z, n) * m


grad_test = test_fun(input, n, input2)
...
# Looks like there is no need to call [input, input2] as done in main branch
