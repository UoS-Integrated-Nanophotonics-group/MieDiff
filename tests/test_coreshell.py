...
def an_sci(x, y, n, m1, m2):
    return (psi(y, n)*(psi_der(m2*y, n) - An_sci(x, n, m1, m2)*chi_der(m2*y, n)) - m2*psi_der(y, n)*(psi(m2*y, n) - An_sci(x, n, m1, m2)*chi(m2*y, n)))/(xi(y, n)*(psi_der(m2*y, n) - An_sci(x, n, m1, m2)*chi_der(m2*y, n)) - m2*xi_der(y, n)*(psi(m2*y, n) - An_sci(x, n, m1, m2)*chi(m2*y, n)))


def bn_sci(x, y, n, m1, m2):
    return (m2*psi(y, n)*(psi_der(m2*y, n) - Bn_sci(x, n, m1, m2)*chi_der(m2*y, n)) - psi_der(y, n)*(psi(m2*y, n) - Bn_sci(x, n, m1, m2)*chi(m2*y, n)))/(m2*xi(y, n)*(psi_der(m2*y, n) - Bn_sci(x, n, m1, m2)*chi_der(m2*y, n)) - xi_der(y, n)*(psi(m2*y, n) - Bn_sci(x, n, m1, m2)*chi(m2*y, n)))
...
class TestCoefficientsForwards(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        for func_ad, func_scipy, kwargs in function_sets:
            ...
            result_ad = func_ad(self.x, self.y, self.n, self.m1, self.m2,)
